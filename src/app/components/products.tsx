...
const Products = () => {
  ...
  return (
    <div className="pt-[120px]">
      <h2 className="text-[#040F16] pb-[40px] text-[48px] text-center  font-semibold">
        Trending products
      </h2>
      <div className="p-[5px] gap-x-[40px] bg-[#14591D]/10 flex rounded-[25px] max-w-[484px]  mx-auto  border border-[#CECECE]">
        ...
        {data.map((item, i) => (
          ...
          >
            ...
            <div className="pt-[30px] px-[5px]">
              <Colors />
              <h3 className="text-[20px] pt-[20px] font-semibold text-heading-color">{item.title}</h3>
              <p className="text-[12px] leading-[140%] text-[#767676]">{item.desc}</p>
            </div>
            <div className="flex pt-[25px] gap-x-[10px] items-center">
                <p className="bg-black  whitespace-nowrap  h-fit text-white text-[14px] font-semibold px-[16px] py-[10px] rounded-full">{item.price}</p>
                <button className="py-[8px] h-[40px]  w-full ease-in-out duration-300 transition-all rounded-full font-medium hover:font-bold hover:text-white hover:border-none border gap-x-[5px] border-[#14591D]/15 flex justify-center items-center text-center bg-white hover:bg-[#14591D] text-heading-color ">
                Buy Now <LeftArrow />{" "}
            </button>
            </div>
          </div>
